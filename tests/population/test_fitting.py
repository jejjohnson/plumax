...
def test_package_exposes_fitting_lazily():
    import plumax.population as population

    assert population.run_mcmc is run_mcmc
    assert population.X_MIN_DEFAULT == X_MIN_DEFAULT
...
def _sample_from_powerlaw(rng, alpha, x_min, x_max, n):
    ...
    return (
        u * (x_max**one_minus_alpha - x_min**one_minus_alpha) + x_min**one_minus_alpha
    ) ** (1.0 / one_minus_alpha)
