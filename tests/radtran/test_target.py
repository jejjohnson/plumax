...
def test_normalized_linear_analytical_form(synthetic_lut):
    ...
    t_expected = (
        -sigma * N * kwargs["delta_vmr"] * kwargs["path_length_cm"] * kwargs["amf"]
    )
...
def test_target_bands_matches_direct_srf_apply(synthetic_lut, swir_srf):
    ...
    t_hr = target_spectrum_normalized_linear(
        synthetic_lut,
        synthetic_lut["wavenumber"].values,
        T_K=280.0,
        p_atm=1.0,
        delta_vmr=1e-6,
        path_length_cm=8.4e5,
    ...
    wl_hr = (
        1e7 / synthetic_lut["wavenumber"].values
    )  # decreasing in ν = increasing in λ? No, λ = 1e7/ν
