...
- **Target signatures** (``target``) — derived from an existing
  :class:`plumax.assimilation.RadianceObservationModel` via
...
def __getattr__(name: str):  # PEP 562 — lazy module attribute access
    try:
        ...
        raise AttributeError(
            f"module 'plumax.matched_filter' has no attribute {name!r}"
        ) from None
    module = importlib.import_module(f"plumax.matched_filter.{submodule}")
...
if TYPE_CHECKING:  # static type checkers see eagerly-imported symbols
    ...
    from plumax.matched_filter.core import (
        apply_image,
        apply_pixel,
        detection_threshold,
        matched_filter_snr,
    ...
    from plumax.matched_filter.io import (
        apply_image_xarray,
        open_multi_scene,
    )
