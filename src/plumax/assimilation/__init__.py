...
:mod:`plumax.radtran` (PEP 562): submodules are imported only on
first attribute access, so ``import plumax`` does not pull in
...
def __getattr__(name: str):  # PEP 562 — lazy module attribute access
    try:
        ...
        raise AttributeError(
            f"module 'plumax.assimilation' has no attribute {name!r}"
        ) from None
    module = importlib.import_module(f"plumax.assimilation.{submodule}")
...
if TYPE_CHECKING:  # static type checkers see eagerly-imported symbols
    ...
    from plumax.assimilation.control import (
        IdentityTransform,
        WhiteningTransform,
    )
    ...
    from plumax.assimilation.obs_operator import (
