...
4. **Type checks** – `uv run --group typecheck ty check src/plumax` (or `make typecheck`) must report no errors in changed files.

> **Common pitfall**: Running `ruff check src/plumax/` instead of `ruff check .` misses lint errors in `tests/` and `scripts/`. CI runs `ruff check .` on the entire repo — always use `.` (repo root), not a subdirectory.
