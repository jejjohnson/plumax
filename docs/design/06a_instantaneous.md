...
*Per-instrument 1σ (log-scale) per-event errors from Sherwin et al. 2024 controlled-release flights.*

| Instrument | Per-event 1σ (log scale) |
| --- | --- |
| TROPOMI ([s5p_tropomi]) | ~0.50 (≈ ±50%) |
| GHGSat ([ghgsat]) | ~0.25 (≈ ±25%) |
| EMIT ([emit]) | ~0.30 |
...
*Catalog ingestion — schema, units, wind source, quality flags per provider.*

| Catalog | Format | Units | Wind source | Quality flags |
| --- | --- | --- | --- | --- |
| IMEO (UNEP) | CSV | t/h | GEOS-FP | provider-supplied |
| Tanager monthly ([carbon_mapper]) | parquet | kg/s | HRRR | confidence tier |
| Carbon Mapper alerts ([carbon_mapper]) | JSON | kg/h | ECMWF | per-pixel mask |
| GHGSat releases ([ghgsat]) | CSV | t/h | 1-km downscaled GHGSat product | binary detection |
...
- **Catalog-vs-full-inversion bias.** Run the full Tier I inversion on a synthetic radiance, compute the $Q$ you'd get from the IME method ([varon2018quantifying]) on the same data. Quantify systematic bias direction (Sherwin et al. 2024 documents this; replicate).
