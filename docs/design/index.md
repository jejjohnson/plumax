...
- **Tier III — Eulerian FV:** 🚧 [`les_fvm`](https://github.com/jejjohnson/plumax/tree/main/src/plumax/les_fvm/) advection/diffusion/dynamics + the strong-constraint **4D-Var loop wired end-to-end** ([`les_fvm/fourdvar.py`](https://github.com/jejjohnson/plumax/tree/main/src/plumax/les_fvm/fourdvar.py)): a differentiable emission→column-obs forward, the 3-term cost in whitened (Matérn-3/2) control space, and an L-BFGS solve whose gradient is the exact discrete adjoint via reverse-mode AD through the diffrax FV solver. Emulator (Step 3) + amortized predictor (Step 5) not started.
