...
- **Tier III — Eulerian FV:** 🚧 [`les_fvm`](https://github.com/jejjohnson/plumax/tree/main/src/plumax/les_fvm/) advection/diffusion/dynamics + the strong-constraint **4D-Var loop wired end-to-end** ([`les_fvm/fourdvar.py`](https://github.com/jejjohnson/plumax/tree/main/src/plumax/les_fvm/fourdvar.py)): a differentiable emission→column-obs forward, the source-prior + time-summed-observation cost in whitened (temporal Matérn-3/2) control space, and an L-BFGS solve whose gradient is the exact discrete adjoint via reverse-mode AD through the diffrax FV solver. v1 inverts a time-resolved scalar emission rate at a known source; the IC/background cost term, spatial source prior, incremental inner solve, emulator (Step 3) and predictor (Step 5) are future work.
