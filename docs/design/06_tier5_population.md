...
| Multi-satellite POD union | `plumax.population.pod.pod_union` | ☐ [#110](https://github.com/jejjohnson/plumax/issues/110) |
| Catalog schema (in-memory cross-tier) | `plumax.population.catalog` (`EmissionEvent` / `EmissionCatalog`) | 🚧 — in-memory Gaussian-summary catalog landed; CSV / parquet ingest [#111](https://github.com/jejjohnson/plumax/issues/111) |
| Real-data CSV ingestion | `plumax.population.ingest` | ☐ [#111](https://github.com/jejjohnson/plumax/issues/111) |
| Population SBC harness | `plumax.population.validation` | ☐ [#114](https://github.com/jejjohnson/plumax/issues/114) |
| Importance-weight ESS diagnostic | `plumax.population.importance.ess_report` | ☐ [#108](https://github.com/jejjohnson/plumax/issues/108) |
...
The `plumax.population` subpackage exists with the v1 core (`catalog`, `size_distribution`, `point_process`). The `methane_pod` library the tables above lean on is **not in this repository** — its *generative* machinery is replaced by a git-pinned dependency on [`xtremax.point_processes`](https://github.com/jejjohnson/xtremax) (which already covers intensities, compensators, thinning samplers, Hawkes, marked processes, a `ThinningProcess` whose retention function is exactly $P_d(Q)$, goodness-of-fit and survival / wait-time primitives) and only the *methane-domain* primitives — POD registry, heavy-tail mark families, missing-mass simulator, fit drivers — are ported in tree under [#106](https://github.com/jejjohnson/plumax/issues/106), and the remaining rows are tracked under the Tier V epic [#75](https://github.com/jejjohnson/plumax/issues/75).
