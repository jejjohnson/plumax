...
This note derives the forward model implemented by `plumax.hapi_lut`: a line-by-line Voigt-profile sum of molecular absorption lines, packaged into a look-up table (LUT) of the absorption cross-section $\sigma(\nu; T, p)$, and applied to a single-layer Beer–Lambert transmittance calculation. The LUT is the precomputed object; the forward model is a $\mathcal{O}(\log N)$ interpolation followed by a single `exp`.
...
This is the single-layer forward model in `plumax.hapi_lut.beers.beers_law_from_lut`.
...
- `beers.plume_ratio_spectrum(ds, vmr_bg, vmr_tot, ...)` — the differential form $\tau_{\mathrm{tot}}(\nu) / \tau_{\mathrm{bg}}(\nu)$ used by the enhancement retrieval in [03_beers_law_with_lut.ipynb](../../notebooks/hapi_lut/03_beers_law_with_lut.ipynb).

The three subsequent notebooks walk through building the LUT ([01](../../notebooks/hapi_lut/01_hapi_lut_ch4.ipynb)), extending to multiple gases ([02](../../notebooks/hapi_lut/02_hapi_lut_multigas.ipynb)), and using it as the forward model of a plume-enhancement retrieval ([03](../../notebooks/hapi_lut/03_beers_law_with_lut.ipynb)).
