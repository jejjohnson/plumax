...
where $Z_i \in \{0,1\}$ is the source's on/off state at scene $i$ (with $\mathbb{E}[Z_i] = \pi$), and $p_i(x,u)$ is scene $i$'s POD ([§II.9.2](02_objects.md#ii-9-2); it varies with wind, viewing geometry, and retrieval sensitivity). The expectation runs over the joint state model of [§III.2](#iii-2), and the answer depends on it:
...
!!! abstract "The central identity"
    ...
    $\Lambda_{\mathrm{fp}}(A)$ is the expected number of false positives in $A$: the clutter process of [§II.9.3](02_objects.md#ii-9-3), superposed on the real detections. Omitting it lets false plumes inflate the inferred source density and the undiscovered population. When individual detections matter, carry latent real/clutter labels instead.
...
=== "(iii) Diffuse"
    ...
    From the coarse-mapper stack; scales at best as $\sigma_r/\sqrt{n_{\mathrm{clear}}}$, and hits a floor under temporally correlated errors ([§II.8.3](02_objects.md#ii-8-3)).
