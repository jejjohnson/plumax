...
Each phase gets the same four headings: **what is random**, the **object** from [Part II](02_inventory_objects.md) that describes it, the **data** it consumes, and what falls out.

(iii-1)=
## III.1 Phase A — Discovery

**What is random**
:   Locations of sources not in the database

**Object**
:   Thinned log-Gaussian Cox process ([§II.7](#ii-7) + [§II.9](#ii-9))

**Data**
:   Fine-imager scenes over searched cells; the infrastructure database; the bottom-up inventory, converted into the GP mean $m_g$ (below)

:::{note} From a bottom-up inventory to $m_g$
A bottom-up product is an emission density $h(x)$ in $\mathrm{kg\,h^{-1}\,km^{-2}}$; $m_g$ is the mean of $g = \log\lambda$, a log *source* density in $\mathrm{km^{-2}}$. Convert in two steps: match expected emission, then correct for the log-normal mean,

$$
\mathbb{E}[\lambda(x)] = \frac{h(x)}{s_0 \int u\,\rho(\mathrm{d}u)},
\qquad
m_g(x) = \log \mathbb{E}[\lambda(x)] - \tfrac12\,k_g(x,x).
$$

This needs a finite-mean $\rho$, such as the tempered GGP ($\tau > 0$).
:::

Discovery counts **sources**, so it needs a finite number of them. Keep $\lambda$ as the CRM base measure of [§II.4](#ii-4) (the same one [§III.3](#iii-3) integrates against), and give each source two marks: its **on-state** rate $s_0 u$, with Lévy measure $\rho(\mathrm{d}u)$, and its persistence $\pi$, with conditional distribution $G(\mathrm{d}\pi \mid u)$. Count only sources above a size floor $u_{\mathrm{floor}}$ (well below $s_{\min}/s_0$). Their count intensity and normalised rate distribution are
...
$$
p_{\mathrm{camp}}(x,u,\pi) = 1 - \mathbb{E}\!\left[\prod_{i=1}^{n} \bigl(1 - Z_i\,p_i(x,u)\bigr)\right],
$$

where $Z_i \in \{0,1\}$ is the source's on/off state at scene $i$ (with $\mathbb{E}[Z_i] = \pi$), and $p_i(x,u)$ is scene $i$'s POD ([§II.9.2](#ii-9-2); it varies with wind, viewing geometry, and retrieval sensitivity). The expectation runs over the joint state model of [§III.2](#iii-2), and the answer depends on it:

- **Independent Bernoulli states** (scenes far apart relative to the on/off timescale): $1 - \prod_i (1 - \pi\,p_i)$, which is $1 - (1 - \pi\,p)^{n}$ for identical scenes.
- **A state that persists through the whole campaign** (a long-lived leak): $\pi\,\bigl[1 - \prod_i (1 - p_i)\bigr]$, which can be far smaller.

Using the independent formula for a strongly correlated source overstates discovery and understates the missed-emission correction. Its mark average is $\bar p(x) = \iint p_{\mathrm{camp}}(x,u,\pi)\,G(\mathrm{d}\pi \mid u)\,f(\mathrm{d}u)$.

:::{important} The central identity
By the (marked) thinning theorem, on a searched cell $A$, **conditional on the intensity** $\lambda$:

$$
\begin{aligned}
N_{\mathrm{det}}(A) \mid \lambda &\sim \mathrm{Poisson}\!\left(\int_A \bar p(x)\,\lambda_{\ge}(\mathrm{d}x) + \Lambda_{\mathrm{fp}}(A)\right),\\
...
$\Lambda_{\mathrm{fp}}(A)$ is the expected number of false positives in $A$: the clutter process of [§II.9.3](#ii-9-3), superposed on the real detections. Omitting it lets false plumes inflate the inferred source density and the undiscovered population. When individual detections matter, carry latent real/clutter labels instead.
...
:class: dropdown
:open:
Choose the floor so that $\lambda_{\ge}(A_1) \approx 15$, the count of [§II.3](#ii-3). With $\bar p \approx 0.3$, the prior mean of detections is $0.3 \times 15 = 4.5$. After one clear fine-imager scene over $A_1$ detecting source 1: $N_{\mathrm{obs}}(A_1) = 1$ against that prior mean of 4.5 detectable, so the posterior $\lambda(A_1)$ drops; the Cox prior propagates milder drops to every cell correlated with $A_1$. $A_2$ and $A_4$ get no direct likelihood term because they are unsearched, but they still move through the kernel: $A_2$ and $A_3$, both adjacent to $A_1$, by similar amounts, and the diagonal $A_4$ by less. Expected undiscovered in $A_1 \approx (1 - \bar p)\,\hat\lambda_{\ge}(A_1)$ with mark-averaged $\bar p \approx 0.3$.
:::

(iii-2)=
## III.2 Phase B — Monitoring

**What is random**
:   The on/off state and timing of *known* sources

**Object**
:   Per-site Beta–Bernoulli persistence ([§II.4.4](#ii-4-4)), a temporal point process of initiations ([§II.10.2](#ii-10-2)), or an on/off switching process ([§II.10.4](#ii-10-4)), observed through the thinned clock ([§II.9.3](#ii-9-3), [§II.10.6](#ii-10-6))
...
| leak until repair | two-state (semi-)Markov switching ([§II.10.4](#ii-10-4)) | on-state duration = repair time |
...
Use the controlled-release detection curve to divide out $p$; otherwise report the product and say so. When the POD varies by scene, do not divide by one curve: fit each clear scene's detection $y_i$ as $\mathrm{Bernoulli}\bigl(\pi_k\,p_i(x_k, s_k)\bigr)$, or, under independent states, use the moment estimate $\hat\pi_k = \sum_i y_i / \sum_i p_i$.
:::

:::{hint} Running example — source 3
:class: dropdown
:open:
10 overpasses, 8 clear, 3 detections at $\approx 400\ \mathrm{kg\,h^{-1}}$. The detection curve gives $p_i(400\ \mathrm{kg\,h^{-1}}) \approx 0.9$ in every clear scene, so the moment estimate is

$$
\hat\pi_3 \approx \frac{\sum_i y_i}{\sum_i p_i} = \frac{3}{8 \times 0.9} \approx 0.42
$$
...
(iii-3)=
## III.3 Phase C — Estimation of rates and total

**What is random**
:   The on-state rates $s_k$, the diffuse density $e(x)$, and their sums

**Object**
:   The full measure $\mu = \mu_{\mathrm{pt}} + \mu_{\mathrm{df}}$ with a heavy-tailed CRM prior on $\mu_{\mathrm{pt}}$ ([§II.4.3](#ii-4-3)), a log-GP prior on $e$ ([§II.6](#ii-6)), and the per-plume observation model ([§II.8.2](#ii-8-2), [§II.9.3](#ii-9-3))

:::{important} Decomposition of the basin total
...
:::
:::{tab-item} (iii) Diffuse
From the coarse-mapper stack; scales at best as $\sigma_r/\sqrt{n_{\mathrm{clear}}}$, and hits a floor under temporally correlated errors ([§II.8.3](#ii-8-3)).
:::
::::
...
|---|---|---|---|
| (i) monitored | $0.9\cdot 120 + 0.3\cdot 410 = 231$ | $\approx 108 + 167 = 275$, $\pm\approx 30\,\%$ per source | source 3 from its [§III.2](#iii-2) estimate $0.42 \times 400$; source 1 held at its true value for brevity |
...
| **total** | **726** | $\approx 675 + \text{(ii)}$ | **(ii) is set by the prior, not the data**; (i) is high by sampling noise on 8 scenes |
...
**Attribution.** When two fine-imager plumes are near each other, DP/Pitman–Yor ([§II.5](#ii-5)) decides "one source or two", with assignment weights built from each candidate's observation rate $\pi_k\,p(x_k, s_k)$, not its emission fraction (see the warning in [§II.5](#ii-5)). Pitman–Yor is the safer choice given the heavy tail.

:::{tip} In `plumax`
The population-level machinery for this phase (TMTPP, POD-corrected totals, the missing-mass paradox) is laid out in [Tier V.D — Total emission estimation](../../design/06_tier5d_total_emission.md).
:::
...
| **rate prior** | CRM (Gamma / GGP) | log-GP on $e$ |
| **persistence prior** | persistence marks $\pi_k$ (Beta–Bernoulli / Beta process) | — (persistent) |
...
(iii-5)=
## III.5 The assumptions carrying the structure

| # | Assumption | Fails when | Repaired by |
|:---:|---|---|---|
| 1 | *Independence across disjoint regions* ([§II.2](#ii-2)) | shared geology | Cox process ([§II.7](#ii-7)) |
| 2 | *Memorylessness in time* ([§II.10.2](#ii-10-2), Poisson) | persistent leaks, schedules, clustered upsets | on/off switching (leaks), renewal (schedules), Hawkes (clustered initiations) |
| 3 | *Point/area is fixed* | always: it is relative to $\Delta x$ ([§II.8.1](#ii-8-1)) | model at the fine scale and coarsen, never the reverse |
| 4 | *Overpasses are a fair sample of source state* ([§II.8.3](#ii-8-3)) | operations correlate with local overpass time | no fix from data alone |

:::{important} The identifiability warning that runs through all three phases
