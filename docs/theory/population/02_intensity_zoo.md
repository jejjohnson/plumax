...
The [intensity gallery notebook](../../notebooks/population/02_intensity_gallery.ipynb) plots every module in this catalog side-by-side using [`plumax.population.intensity`](xref:api#plumax.population.intensity).
...
$$
\lambda(t) = \exp\big(f(t)\big), \qquad f(t) \sim \mathcal{GP}\big(0, K(t, t')\big)
$$
...
$$
\frac{\mathrm{d}z(t)}{\mathrm{d}t} = \alpha\,\big(1 + \sin(\omega t)\big) - \beta\, z(t), \qquad \lambda(t) = \ln\big(1 + e^{z(t)}\big)
$$
...
$$
h(t) = \operatorname{RNN}\big(h(t_{i-1}),\, t - t_{i-1}\big), \qquad \lambda(t) = f\big(W^\top h(t) + b\big)
$$
