...
* **The Equation:** $\mathbb{E}[\Delta t \mid t_0] = \int_{t_0}^\infty \exp\!\left(-\int_{t_0}^{t} \lambda_\text{true}(u)\,\mathrm{d}u\right) \mathrm{d}t$
...
* **Homogeneous:** $P\big(N(T) \ge 1\big) = 1 - e^{-\lambda_\text{true} T}$
...
* **The Equation:** $P\big(N(t_1, t_2) \ge 1\big) = 1 - \exp\!\left(-\int_{t_1}^{t_2} \lambda_\text{true}(t)\,\mathrm{d}t\right)$
...
* **Homogeneous:** $S(t) = e^{-\lambda_\text{true} t}$
...
* **The Equation:** $S(t \mid t_0) = \exp\!\left(-\int_{t_0}^{t} \lambda_\text{true}(u)\,\mathrm{d}u\right)$
...
* **The Equation (With Memory):** $h(t) = f_\text{time}(t) / S(t)$ *(where `f_time(t)` is the PDF of the wait times).*
...
* **Homogeneous Marks:** $\mathbb{E}[M_\text{event}] = \left[\int_0^\infty Q\, f_\text{true}(Q)\,\mathrm{d}Q\right] \mathbb{E}[D]$ *(where `E[D]` is duration in hours).*
...
* **The Equation:** $\mathbb{E}[M_\text{event}(t)] = \mathbb{E}[D] \left[\int_0^\infty Q\, f_\text{true}(Q, t)\,\mathrm{d}Q\right]$
...
* **Homogeneous:** $\mathbb{E}[M_\text{total}] = \lambda_\text{true}\, T\, \mathbb{E}[M_\text{event}]$
...
* **The Equation:** $\mathbb{E}[M_\text{total}] = \left[\int_0^T \lambda_\text{true}(t)\,\mathrm{d}t\right] \mathbb{E}[M_\text{event}]$
...
* **The Equation:** $P(Q > Q_\text{crit}) = \int_{Q_\text{crit}}^\infty f_\text{true}(Q)\,\mathrm{d}Q$
...
In naive systems, operators rely on binary logic: *If I see a leak, it is broken. If I do not see a leak, it is fixed.* By explicitly calculating the Expected Probability of Detection ($\mathbb{E}[P_d] = \int_0^\infty P_d(Q)\, f_\text{true}(Q)\,\mathrm{d}Q$), you mathematically quantify your blindness. If `E[P_d]` collapses to 5% because the region has high wind shear, dense cloud cover, or a low-albedo background (like dark water or snow), your software actively forbids the stakeholder from labeling the site as "Mitigated."
...
* **The Equation:** $\lambda = n p$
* **Rearranged for p:** $p = \lambda / n$

Now, we calculate the Binomial probability of seeing exactly `k` leaks in `n` total pictures.

* **The Binomial Equation:** $P(X = k) = \binom{n}{k} p^k (1 - p)^{n - k}$
