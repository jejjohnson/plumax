...
This note derives the cost function used by `plumax.assimilation`, walks through the three control-variable formulations we implement, and then maps each piece of the math onto a concrete `jax` / `lineax` / `optimistix` / `gaussx` call. The aim is that you can read this side-by-side with [solve.py](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/solve.py) and see exactly which line implements which equation.
...
> **In code.** Equation [eq-J](#eq-J) is built by [`build_cost_x`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/cost.py); the prior term uses `gaussx.solve(B, δx_flat)` so any structured `B` from [background.py](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/background.py) — diagonal, Kronecker, low-rank — is plugged in without materialising. The obs term differentiates through the JAX `forward_fn` via reverse-mode AD, so we never write `H'^\top` by hand.
...
> **In code.** Equation [eq-J-whitened](#eq-J-whitened) is built by [`build_cost_xi`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/cost.py); the operator $U$ is constructed by [`WhiteningTransform.from_background`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/control.py), which calls `gaussx.cholesky(B)`. Crucially, gaussx returns a *structured* operator — a Cholesky of a `Kronecker` is itself a Kronecker of Cholesky factors — so all subsequent matvecs and inverse-solves stay in the cheap regime.
...
> **In code.** [`run_dual_psas`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/solve.py) wraps the dual matrix as a `lineax.FunctionLinearOperator`, tags it symmetric+PSD, and feeds it to `lineax.linear_solve(..., solver=lineax.CG(...))`. The two `H'` and $H'^\top$ applications inside the matvec are produced by differencing `forward_fn` at `x_b` and by a single hoisted `jax.vjp` respectively — *no explicit adjoint of the obs operator anywhere in the code.*
...
So matched filter, narrow-band retrieval, and full 3D-Var are *the same equation* with progressively richer prior assumptions. The matched-filter implementation in [matched_filter.py](https://github.com/jejjohnson/plumax/blob/main/src/plumax/radtran/matched_filter.py) and the `gaussx`-backed variant in [gaussx_solve.py](https://github.com/jejjohnson/plumax/blob/main/src/plumax/radtran/gaussx_solve.py) drop out as the simplest case of the framework here.
...
This is what we use for the obs-term gradient in [eq-J](#eq-J). Concretely, [`build_cost_xi`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/cost.py) wraps the cost in `jax.value_and_grad(value)`; reverse-mode AD walks back through `forward_fn`, applying each elementary VJP — including the kernel-flipped convolution adjoint inside [`PointSpreadFunction`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/radtran/instrument.py) and the spread-with-1/f² adjoint inside [`GroundSamplingDistance`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/radtran/instrument.py). The unit tests in [test_instrument.py](https://github.com/jejjohnson/plumax/blob/main/tests/radtran/test_instrument.py) verify the inner-product identity $\langle A x, u \rangle = \langle x, A^\top u \rangle$ for both operators, so the autodiff gradient is provably equal to what a hand-rolled adjoint would give.
...
- The dual matvec inside [eq-dual-solution](#eq-dual-solution): one `jvp` per CG iteration (we use this implicitly via `forward_fn` differenced at `x_b` in [`run_dual_psas`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/solve.py)).
- Hessian-vector products via forward-over-reverse: `jax.jvp(jax.grad(J), (x,), (v,))` returns $\nabla^2 J \cdot v$ at $\mathcal{O}(\text{forward})$ cost. This is what [`Cost.hvp`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/cost.py) does, and it's what feeds the diagnostics in [diagnostics.py](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/diagnostics.py).
...
> **In code.** All of our solvers in [solve.py](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/solve.py) accept the optimistix default `adjoint=ImplicitAdjoint()`; you can override it with a custom `linear_solver` (e.g., `lineax.CG(rtol=...)` for matrix-free, or `lineax.Cholesky()` for small dense Hessians) when you start computing sensitivities of the analysis with respect to its inputs.
...
| $\chi^2_\text{red}$, DFS, posterior cov    | [`reduced_chi_squared`, `degrees_of_freedom_for_signal`, `posterior_covariance_proxy`](https://github.com/jejjohnson/plumax/blob/main/src/plumax/assimilation/diagnostics.py) |

## 6. Worked example

The end-to-end demo lives in [06_variational_methane_retrieval.ipynb](../../notebooks/assimilation/06_variational_methane_retrieval.ipynb). It builds a synthetic plume from the HAPI LUT, injects it via the JAX forward operator, and runs the retrieval three ways — naive primal, whitened LBFGS, and dual PSAS — comparing iteration counts, run times, and posterior diagnostics. The same notebook also shows the matched filter from PR #24 falling out of the dual recipe with a single CG iteration.
