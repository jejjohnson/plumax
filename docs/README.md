...
`/plumax/plumax/reference/` — and 404s.
...
[`MetField`](xref:api#plumax.met.MetField)
```
...
|---|---|---|
| `plumax.met.MetField` -> `met/#$` | `#plumax.met.metfield` | **no** |
| `plumax.gauss_plume.simulate_plume` -> `gauss_plume/#$` | `#plumax.gauss_plume.simulate_plume` | yes (already lower case) |

The failure is silent: the link resolves, the page loads, and the browser
simply cannot find the anchor.

**This does bite in plumax.** Each sub-package is documented on its own page,
so every inventory entry uses `$`, and any link to a class or other
mixed-case name comes out lowercased. Two safety nets handle it in
